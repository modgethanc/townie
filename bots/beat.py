...
def internettime(hours, minutes, seconds, tzone):
    ...
    '''
    itime = (((seconds + (minutes * 60) + ((hours + tzone) * 3600)) / 86.4)
            % 1000)
    beats = int(itime)
...
def main():
    ...
    if args.version:
        return('.beat v{0}'.format(pkg_resources.get_distribution("beat").version))
    elif args.time:
        hours, minutes, seconds = args.time.split(":")
        return internettime(int(hours), int(minutes), int(seconds),
                           args.timezone)
    else:
